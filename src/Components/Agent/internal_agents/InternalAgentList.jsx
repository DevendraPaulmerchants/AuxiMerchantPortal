...
function InternalAgentList({ searchText = '', agentwithAccStatus }) {
    ...
    const handleStatusChange = (Id) => {
        const confirm = window.confirm("Are you sure you want to update this agent's status?");
        if (!confirm) return;
        fetch(`${APIPath}merchant-service/internal-agent/${Id}`, {
            method: 'PATCH',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
            ...
            .then((res) => res.json())
            .then((data) => {
                alert(data.message || 'Agent`s Account status Changes successfully');
                getInternalAgentList()
            })
            .catch((err) => {
    ...
    return (
        <>
            ...
            {selectedRow &&
                <InternalAgentForm close={closeEditedAgent} selectedRow={selectedRow} updateList={getInternalAgentList} />
            }
        </>
    )
