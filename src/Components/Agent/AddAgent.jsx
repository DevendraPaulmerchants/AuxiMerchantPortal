...
import { handleEmailChange, handleInputChangeWithAlphabetOnly, handleInputChangeWithNumericValueOnly, handlePANCardNumber, isValidAadhaar, panData, urlToFile } from '../InputValidation/InputValidation';
import { APIPath } from '../ApIPath/APIPath';
import { useContextData } from '../Context/Context';
import { FcCancel, FcOk } from 'react-icons/fc';
import AddInternalAgent from './internal_agents/AddInternalAgent';

function AddAgent({ close, selectedAgent, updateList }) {
  useEffect(() => {
    console.log("New Agent Creating...")
  ...
  const [previewAgentImage, setPreviewAgentImage] = useState('');

  const [agentShopImage, setAgentShopImage] = useState('');
  ...
  // State to manage the selected type of agent (internal or external)
  ...
  // Agent Images --------------------
  const handleAgentImageFile = (e) => {
    const file = e.target.files[0];
    if (file) {
      setAgentImage(file);
      const fileUrl = URL.createObjectURL(file);
      setPreviewAgentImage(fileUrl);
    }
  }

  // Agent Shop Image ----------------
  const handleAgentShopImageFile = (e) => {
    const file = e.target.files[0];
    if (file) {
      setAgentShopImage(file);
      const fileUrl = URL.createObjectURL(file);
      setPreviewShopImage(fileUrl);
    }
  }
  ...
  const handleFormData = (e) => {
    ...
    formData.append('shopImage', agentShopImage);
  ...
  return <>
    <div className={style.add_merchants_parent}>
      <div className={style.add_merchants_form_container}>
        ...
        {/* --------------- Internal External Button Container ---------- */}
        <div className={style1.internal_external_button_container}>
          <div className={style1.external_agent}>
            <input
              type="checkbox"
        ...
        {/* ----------------- Internal Agent Form ------------------ */}
        {selected === 'internal' ?
          <AddInternalAgent close={close} />
          :
          /*----------- Add External Agent ------------  */
          <form onSubmit={(e) => {
            handleFormData(e)
          }}>
            {/* -------- Agent Name and Brand Name ------------ */}
            ...
            {/* ------------ Upload Agent Image and Shop Image ---------- */}
            <div className={style.name_email_parent_container}>
              <div className={style.name_label_input_contaner}>
                <label>Upload Agent Image* </label>
                <input type='file' accept="image/*,application/pdf"
                  required={selectedAgent?.aadhaar_image_url ? false : true}
                  onChange={handleAgentImageFile}
                />
                <p style={{ fontSize: '10px' }}>Kindly upload your document as a Image/PDF file only.</p>
                {previewAgentImage &&
                  <div className={style1.file_mini_view}>
                    <iframe className={style.mini_file}
                      src={previewAgentImage}
                      title="File Preview"
                      width='100%'
                      height='100px'
                      frameBorder="0">
                    ...
                    <p className={style1.max_view}
                      onClick={() => {
                        setSelectedUrl(previewAgentImage);
                        setMaxView(true);
                      }}
                    >
                      <CgMaximizeAlt />
              ...
              <div className={style.name_label_input_contaner}>
                ...
                <input type='file' accept="image/*,application/pdf"
                  ...
                  onChange={handleAgentShopImageFile}
                />
                <p style={{ fontSize: '10px' }}>Kindly upload your document as a Image/PDF file only.</p>
                {previewShopImage &&
                  <div className={style1.file_mini_view}>
                    ...
                    <p className={style1.max_view}
                      onClick={() => {
                        setSelectedUrl(previewShopImage);
                        setMaxView(true);
                      }}
                    >
                      <CgMaximizeAlt />
            ...
            <div className={style.name_email_parent_container}>
              ...
              <div className={style.name_label_input_contaner}>
                ...
                <div className={style1.panNumber_with_validation_icon}>
                  <input type='text' placeholder='Enter PAN number' minLength={10} maxLength={10} value={panNumber}
                    onChange={(e) => {
                      handlePANCardNumber(e, setPANnumber, setIsValidPAN, setLoader, setPanData)
                    }}
                  />
                  {loader && <p className={style1.pan_loader}></p>}
                  {!loader && panNumber.length === 10 && panData !== null
                    ? (panData.status === 'Active'
                      ? <FcOk title="Verified" />
                      : <FcCancel title="Not Valid" />)
                    : null}
                </div>
                {!loader && panNumber.length === 10 && panData !== null
                  ? (panData.status === 'Active'
                  ...
                  : null}
                {panNumber.length > 0 && !isValidPAN && <p className={style.not_valid_text}>please enter valid PAN number</p>}
              </div>
            </div>
            {/* ------------ Upload Document ------------------------------------  */}
            <div className={style.add_merchants_bussiness_address}>
