...
function SilverTransaction() {
  ...
  useEffect(() => {
    setIsLoading(true)
    fetch(`${APIPath}merchant-service/history/merchant-transaction-history/${merchantId}?metalType=SILVER&startDate=${startDate?startDate:''}&endDate=${endDate?endDate:''}`, {
      headers: {
      ...
      .then((data) => {
        ...
        if (transactionType === "SELL") {
          const sell = data.data.filter((data) => data.order_type === "SELL")
          setGoldLIst(sell);
        }
        else if (transactionType === "BUY") {
          const buy = data.data.filter(item => item.order_type === "BUY");
          setGoldLIst(buy);
        }
        else if (transactionType === "TRANSFER") {
          const transfer = data.data.filter(item => item.order_type === "TRANSFER");
          setGoldLIst(transfer);
        }
        else if (transactionType === "CONVERSION") {
          const conversion = data.data.filter(item => item.order_type === "CONVERSION");
          setGoldLIst(conversion);
        }
        else {
  ...
  }, [transactionType, token, merchantId, startDate, endDate])

  const filteredList = goldList?.filter((list) => list.metal_type?.toLowerCase().includes(searchText.toLowerCase()));
