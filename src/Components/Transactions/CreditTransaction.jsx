...
function CreditTransaction() {
  ...
  const filteredList = goldList?.filter((list) => {
    if (transactionType === 'DEBIT' && list.transactionType !== 'DEBIT') return false;
    if (transactionType === 'CREDIT' && list.transactionType !== 'CREDIT') return false;
    return list.id?.toLowerCase().includes(searchText.toLowerCase())
  });
